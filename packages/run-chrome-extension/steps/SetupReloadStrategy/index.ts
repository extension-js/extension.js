...
class SetupReloadStrategy {
  ...
  public apply(compiler: webpack.Compiler) {
    // Ensure the background scripts (and service_worker) can
    // receive messages from the extension reload plugin.
    AddRuntimeListener(compiler, this.manifestPath)

    // Patch the manifest with useful transforms during development,
    ...
    // Add the HMR reloader to the entry point.
