...
async function showUserDistStructure() {
  ...
  try {
      ...
      for (const dir of directories) {
        ...
        if (dir.name.includes('-manager-')) {
          ...
          try {
            ...
            const reloadContent = await fs.promises.readFile(
              reloadServicePath,
              'utf-8'
            )

            const portMatch = reloadContent.match(
              /const\s+port\s*=\s*['"](\d+)['"]/
            )
            const instanceMatch = reloadContent.match(
            ...
            if (instanceMatch) {
              console.log(
                `      🆔 Instance ID: ${instanceMatch[1].slice(0, 8)}`
              )
            }
          } catch (error) {
    ...
    for (const example of examples) {
      const exampleDistPath = path.join(
        __dirname,
        `examples/${example}/dist/extension-js/extensions`
      )

      if (fs.existsSync(exampleDistPath)) {
        const entries = await fs.promises.readdir(exampleDistPath, {
          withFileTypes: true
        })
        const managerDirs = entries.filter(
          (entry) =>
        ...
        if (managerDirs.length > 0) {
          console.log(
            `   🔵 ${example}: ${managerDirs.length} manager extension(s)`
          )
