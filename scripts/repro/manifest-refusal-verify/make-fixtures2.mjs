...
const fixtures = [
  ...
  // non-string name
  {
    id: '26-name-nonstring',
    expect: 'refuse',
    manifest: {manifest_version: 3, name: 42, version: '1.0'}
  },
  // non-string run_at
  {
    id: '27-run-at-nonstring',
    expect: 'refuse',
    manifest: {
      ...base('f27'),
      content_scripts: [{matches: ['<all_urls>'], js: ['c.js'], run_at: 3}]
    },
    files: {'c.js': '// noop'}
  },
  // re-verify the existing 0-byte icon assumption on this build
  {
    id: '28-icon-zero-byte',
    expect: 'refuse',
  ...
  {
    id: '29-msg-unresolved-description',
    expect: 'refuse',
    manifest: {
      ...base('f29'),
      description: '__MSG_missing__',
      default_locale: 'en'
    },
    files: {'_locales/en/messages.json': msgs({x: {message: 'y'}})}
  },
