...
for (const batch of batches) {
  ...
  for (const {id, expect} of index) {
    ...
    const proc = spawnSync('xcrun', [
      ...
      '--app-name', `V${id.replace(/[^a-zA-Z0-9]/g, '')}`,
      '--bundle-identifier', `dev.extensionjs.verify.${id.replace(/[^a-zA-Z0-9]/g, '')}`,
      '--macos-only',
      '--no-open',
      '--no-prompt',
      '--force'
    ...
    const interesting = output
      .split(/\r?\n/)
      .filter((line) => /error|warning|unable|invalid|missing|failed/i.test(line))
      .slice(0, 4)
      .map((line) => line.trim())
...
fs.writeFileSync(path.join(here, 'results-safari.json'), JSON.stringify(results, null, 2))
