...
describe('bundled templates keep documentation art out of the store payload', () => {
  ...
  it.each(
    bundledTemplates()
  )('%s ships no screenshot under public/, which the build copies verbatim', (template) => {
  ...
  it.each(
    bundledTemplates()
  )('%s keeps every public/ asset small enough to belong in a shipped extension', (template) => {
    const publicDir = path.join(TEMPLATES_DIR, template, 'public')
  ...
  it.each(
    bundledTemplates()
  )('%s README points at a screenshot that exists where it points', (template) => {
