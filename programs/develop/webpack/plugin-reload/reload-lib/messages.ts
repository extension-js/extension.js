...
export function extensionData(
  ...
  if (!message.data) {
    ...
    return `[⛔️] ${bgWhite(bold(` ${browser}-browser `))} ${red('✖︎✖︎✖︎')} No data received from client.
...
Ensure your extension is enabled and that no hanging browser instance is open then try again.`;
  ...
  const { name, description, version, hostPermissions, permissions } =
    management;
...
${bold(`• Permissions:`)} ${permissionsParsed.sort().join(', ') || 'Browser defaults'}
${bold(`• Settings URL:`)} ${underline(blue(`${browser}://extensions/?id=${id}`))}\n`;
}
...
export function stdoutData(
  ...
  return (
    `${crRuntime} ${green('►►►')} Running ${capitalize(browser)} in ${bold(modeColor(compilerOptions.mode || 'unknown'))} mode. ` +
    `Browser ${management?.type} ${bold(management?.enabled ? 'enabled' : 'disabled')}.`
  );
}
...
export function watchModeClosed(code: number, reason: Buffer) {
  ...
  return `[😓] ${bgWhite(bold(` chrome-browser `))} ${red('✖︎✖︎✖︎')} Watch mode closed (code ${code}). ${message && '\n\nReason ' + message + '\n'}Exiting...\n`;
}

export function webSocketError(error: any) {
  return `[⛔️] ${bgWhite(bold(` chrome-browser `))} ${red('✖︎✖︎✖︎')} WebSocket error: ${error}`;
}
