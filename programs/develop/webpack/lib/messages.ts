...
${bold(`• Host Permissions:`)} ${hasHost ? hostPermissions.sort().join(', ') : 'Browser defaults'}
${bold(`• Permissions:`)} ${hasPermissions ? permissions.sort().join(', ') : 'Browser defaults'}
  `;
}
...
export function getAssetInfo(
  ...
  assets?.forEach((asset) => {
    ...
    output += `• ${bold('Filename:')} ${yellow(asset.name)}, ${bold('Size:')} ${sizeInKB}\n`;
    output += `  ${bold('└─')} ${underline(`${path.join(outputPath, asset.name)}`)}\n`;
  });
  return output;
}
