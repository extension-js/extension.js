...
export function missingSassDependency() {
  return [
    `${prefix('error')} SASS support requires the ${colors.brightBlue(
      '"sass"'
...
export function cssParseErrorShippedVerbatim(
  ...
  return [
    `${prefix('warn')} Invalid CSS in ${colors.underline(resourcePath)}, ${reason}.`,
...
export function preprocessorShippedUncompiled(
  ...
  return [
    `${prefix('warn')} ${colors.underline(resourcePath)} shipped UNCOMPILED, ${colors.brightBlue(`"${pkg}"`)} is not installed in this project.`,
...
export function deadCssUrlRef(issuerPath: string, request: string) {
  return [
    ...
    `Chrome applies the rest of the stylesheet and 404s this reference silently, likely dead code.`,
