...
/**
 * Prepends the agent-bridge RELAY to compiled content scripts AND surface pages
 * (popup/options/sidebar/devtools) so each forwards its console to the background
 * SW — which owns the control-WS — via chrome.runtime.sendMessage. The SW relay
 * stamps tabId/url and ships it over the WS. Multi-context console forwarding
 * (Slice 3): "one stream from every context."
 *
 * Mirrors InjectBridgeProducer (same asset-level post-process, same env-gating):
 * only injects when the control bridge is available (EXTENSION_CONTROL_PORT set).
 * The relay opens no socket of its own.
 */
