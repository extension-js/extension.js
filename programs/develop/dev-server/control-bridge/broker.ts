...
/**
 *. How long after session start to stay quiet about a reload that reached
...
/**
 *. Operator-facing warnings for a dev-loop reload that reached no page,
...
export class BridgeBroker {
  ...
  /** Broker clock at session start, the anchor for's startup grace. */
  private readonly sessionStartedAt: number
  /**
   * dedup: the absence kind we last warned about for an undelivered reload.
  ...
  /**
   *. After a dev-loop {@link broadcastReload} reached zero producers, decide
  ...
  private onHello(conn: BridgeConnection, hello: HelloFrame): void {
    ...
    if (hello.role === 'producer') {
      ...
      // A producer is back: clear the undelivered-reload dedup so that if it
