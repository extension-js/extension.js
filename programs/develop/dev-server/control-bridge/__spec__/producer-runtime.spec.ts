...
describe('bridge producer runtime, executor (Slice 2)', () => {
  ...
  it('eval content resolves --url to the matching tab id (#51)', async () => {
    ...
    const ws = setup({
      scripting: {
        // Run the injected func like Chrome would, so the wrapper
  ...
  it('eval content: a null injection result is an error, never ok:true null', async () => {
  ...
  it('eval popup with the surface closed reports Unsupported, not a tabId demand', async () => {
    const ws = setup({
      runtime: {
  ...
  it('tabs.query works on a callback-only chrome.* (Gecko MV2)', async () => {
    // Gecko's chrome.tabs.query returns undefined (callback-only): the
  ...
  it('tabs.query prefers the promisified browser.* namespace when present', async () => {
  ...
  it('the relay answers a surface inspect request for its own context', () => {
    ...
    // One listener for surface eval, one for surface inspect.
