...
describe('BridgeBroker (Slice 1: logs)', () => {
  ...
  it('sends a ready frame to a consumer on hello', () => {
    ...
    expect(c.sent[0]).toMatchObject({type: 'ready', runId: 'run-A', engine: 'chromium'})
  })
  ...
  it('fans a producer log out to consumers with a stamped seq', () => {
    ...
    b.onFrame(cons, {type: 'hello', v: 1, role: 'consumer', instanceId: 'inst-1'})
  ...
  it('replays the retained ring to a late consumer', () => {
    ...
    b.onFrame(late, {type: 'hello', v: 1, role: 'consumer', instanceId: 'inst-1'})
    // ready + 2 replayed logs
    expect(late.sent[0]).toMatchObject({type: 'ready', bufferedFrom: 1})
    expect(late.sent.slice(1).map((f: any) => f.event.messageParts[0])).toEqual(['a', 'b'])
  })
  ...
  it('emits a gap frame when the ring overflows', () => {
    ...
    expect(gaps[0]).toMatchObject({type: 'gap', dropped: 1, reason: 'ring_overflow'})
  })
