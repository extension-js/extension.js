...
export const BRIDGE_PRODUCER_SOURCE = `;(function () {
  try {
    ...
    // chrome.* async APIs are callback-only on Gecko (the/ TypeError:
      ...
      try {
        ...
        if (op === "eval") {
          if (ctx === "background") {
            ...
            // while nothing ever executed. The common throw is the
            ...
            // own in-bundle relay, mirroring inspect. Only the open,
            // matching-context page responds. url-override pages route
        ...
        if (op === "storage.get" || op === "storage.set") {
          ...
          // is the TypeError. Prefer the promisified browser.* namespace,
        ...
        if (op === "inspect") {
          ...
          if ((ctx === "content" || ctx === "page") && target.tabId && chrome.scripting) {
            ...
            // url-override pages are surfaces too.
            chrome.runtime.sendMessage(
    ...
    // uncaught errors and unhandled rejections don't route through the
    ...
    // background crash is visible in \`extension logs\`. Deduped against the
...
export const BRIDGE_RELAY_SOURCE = `;(function () {
  try {
    ...
    // uncaught throws / unhandled rejections in a page or content script
    ...
    // Surface eval: like inspect below, the SW executor can't run code in
