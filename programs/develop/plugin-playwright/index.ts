...
// advertising status:"ready" to controllers (/ honesty).
...
export function createPlaywrightMetadataWriter(options: WriterOptions) {
  ...
  // a second command (build/preview/start) against a project whose
  ...
  return {
    ...
    // green "ready" over a dead pid (/). Read-modify-write to keep the
...
export class PlaywrightPlugin {
  ...
  apply(compiler: Compiler) {
    ...
    compiler.hooks.watchClose.tap(PlaywrightPlugin.name, () => {
      ...
      // is about to be gone; controllers then dial a dead control port.
