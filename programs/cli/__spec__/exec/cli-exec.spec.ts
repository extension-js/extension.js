...
describe.each(availableRunners)('cli exec flow (%s)', (runner) => {
  ...
  it('creates default template without install', () => {
    ...
    try {
      const result = runCommand(
        runner.command,
        runner.buildArgs(packages, ['create', projectPath]),
  ...
  it('fails on invalid template', () => {
    if (!runnerReady.get(runner.name)) return
    const workspace = mkdtempSync(join(tmpdir(), 'extjs-cli-create-'))
    const projectPath = join(workspace, 'app-invalid')
    try {
      const result = runCommand(
        runner.command,
        runner.buildArgs(packages, [
  ...
  it('starts without install when project has no deps', () => {
    if (!runnerReady.get(runner.name)) return
    const workspace = mkdtempSync(join(tmpdir(), 'extjs-cli-noinstall-'))
    const projectPath = join(workspace, 'app-start')
    try {
      const createResult = runCommand(
        runner.command,
        runner.buildArgs(packages, ['create', projectPath]),
      ...
      const startResult = runCommand(
        runner.command,
        runner.buildArgs(packages, ['start', projectPath, '--no-runner']),
        {cwd: projectPath, env: defaultEnv}
      )
  ...
  it('runs dev without install when project has no deps', async () => {
    ...
    try {
      ...
      const devResult = await runUntilTimeout(
        runner.command,
        runner.buildArgs(packages, ['dev', projectPath]),
        {cwd: projectPath, env: defaultEnv},
        6000
      )
  ...
  it('installs local tarballs and can build', () => {
    ...
    try {
      ...
      const buildResult = runCommand(
        runner.command,
        runner.buildArgs(packages, ['build', projectPath, '--silent', 'true']),
        {cwd: projectPath, env: {...defaultEnv, EXTENSION_AUTHOR_MODE: 'true'}}
      )
      expect(buildResult.status).toBe(0)
    } finally {
      rmSync(workspace, {recursive: true, force: true})
  ...
  it('starts dev server and can be terminated', async () => {
    if (!runnerReady.get(runner.name)) return
    const workspace = mkdtempSync(join(tmpdir(), 'extjs-cli-dev-'))
    const projectPath = join(workspace, 'app-dev')
    try {
      const createResult = runCommand(
        runner.command,
  ...
  it('starts preview via start --no-runner after install', () => {
    if (!runnerReady.get(runner.name)) return
    const workspace = mkdtempSync(join(tmpdir(), 'extjs-cli-start-'))
    const projectPath = join(workspace, 'app-start')
    try {
      const createResult = runCommand(
        runner.command,
