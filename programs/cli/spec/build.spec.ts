...
import {ALL_TEMPLATES, BROWSERS, CUSTOM_TEMPLATES} from './constants'
...
describe('extension build', () => {
  ...
  //   async (template) => {
  //     const templatePath = path.join(__dirname, 'fixtures', template)
  //     const templateDistPath = path.join(__dirname, 'fixtures', template, 'dist', BROWSERS[0])
  ...
  //     expect(
  //       fs.existsSync(path.join(templateDistPath, 'side_panel', 'default_path.css'))
  //     ).toBeTruthy()
  //     expect(
  //       fs.existsSync(
  ...
  it.each([CUSTOM_TEMPLATES])(
    `builds and zips the distribution files of an extension created via "%s" template using the --zip flag`,
    async (template) => {
      const templatePath = path.join(__dirname, 'fixtures', template)
  ...
  it.each([CUSTOM_TEMPLATES])(
    `builds and zips the source files of an extension created via "%s" template using the --zip-source flag`,
    async (template) => {
      const templatePath = path.join(__dirname, 'fixtures', template)
      const templateDistPath = path.join(
        __dirname,
